...
Authentication Method | In Cluster | Outside Cluster | No User Interaction
--- | :---: | :---: | :---:
...
!!! info "Refresh Token Expiry"
    ...
    - [`deploykf_core.deploykf_auth.dex.expiry.refreshToken.idle`](https://github.com/deployKF/deployKF/blob/v0.1.2/generator/default_values.yaml#L429)
    - [`deploykf_core.deploykf_auth.dex.expiry.refreshToken.total`](https://github.com/deployKF/deployKF/blob/v0.1.2/generator/default_values.yaml#L432)

First, we define the `DeployKFCredentialsOutOfBand()` class which extends `TokenCredentialsBase()` to create a custom credential provider:
...
Next, we create a `kfp.Client()` which uses the `DeployKFCredentialsOutOfBand()` class for authentication:
...
!!! note "Provision Static Credentials"

    Dex static credentials are managed by config values and are provisioned by the cluster administrator.
    The [user authentication admin guide](../guides/platform/deploykf-authentication.md#static-userpassword-combinations) provides information about managing static credentials in deployKF.
...
The following reference implementation shows how to authenticate the _Kubeflow Pipelines SDK_ using a Kubernetes ServiceAccount token.
...
=== "Pod Definition (Manual)"
    ...
    The following Pod will have a ServiceAccount token volume mounted at the path `/var/run/secrets/kubeflow/pipelines/token`:
...
=== "PodDefault Injection (Automatic)"

    In addition to manually mounting the ServiceAccount token volume, deployKF can automatically inject the volume using a [PodDefault](https://github.com/kubeflow/kubeflow/tree/master/components/admission-webhook) resource based on a Pod's labels.

    When the [`kubeflow_tools.pipelines.profileResourceGeneration.kfpApiTokenPodDefault`](https://github.com/deployKF/deployKF/blob/v0.1.2/generator/default_values.yaml#L1804-L1812) value is `true`, a PodDefault named `"kubeflow-pipelines-api-token"` is automatically created in each profile namespace.
    This means that any Pod with the `kubeflow-pipelines-api-token=true` label will automatically have a ServiceAccount token volume mounted to the Pod.

    !!! note "Kubeflow Notebooks Integration"
    ...
    ??? question_secondary "How can I define my own PodDefault?"
...
The following Python code creates a `kfp.Client()` using a ServiceAccount token for authentication (when run from inside the cluster):
...
??? question_secondary "How can I specify a different token path or host?"
